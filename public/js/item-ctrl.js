/* global angular */
// TODO this is *very* similar to patron-ctrl.js - combine/use template

angular.module('libraryApp')
.controller('ItemCtrl', function($scope, $http) {
  this.perPage = 30;
  this.sortBy = 'created';
  this.sortDir = 'asc';
  
  let computeMaxPage = () => {
    return Math.ceil(this.numItems / this.perPage) - 1;
  };
  
  // GET the items and put in this.items
  this.getItems = (page = 0, callback) => {
    this.page = page;
    
    $http.get(`/v0/items?page=${page}&per_page=${this.perPage}`
            + `&sort_by=${this.sortBy}&direction=${this.sortDir}`)
    .then(res => {
      this.items = res.data.data;
      this.numItems = res.data.maxItems;
      this.maxPage = computeMaxPage();
      
      if (res.status === 206) {
        let range = res.headers('Range').split('/')[0];
        let [low, high] = range.split('-');
        
        this.lowIndex = parseInt(low);
        this.highIndex = parseInt(high);
      } else {
        this.lowIndex = 0;
        this.highIndex = this.numItems - 1;
      }
      
      if (callback) callback();
    }, () => {
      this.items = []; // Error!
    });
  };
  this.getItems();
  
  // Handle changing per page and sorting
  
  this.changePerPage = perPage => {
    this.perPage = perPage;
    let newPage = Math.min(this.page, computeMaxPage());
    this.getItems(newPage);
  };
  
  this.changeSortBy = sortBy => {
    this.sortBy = sortBy;
    this.getItems(this.page);
  };
  
  this.changeSortDir = sortDir => {
    this.sortDir = sortDir;
    this.getItems(this.page);
  };
  
  // Handle deleting ALL THE THINGS
  this.delAll = () => {
    let seriously = confirm('This will delete ALL ITEMS! '
      + 'This cannot be undone! Are you sure you want to do this?');
    if (!seriously) return;
    
    let honestly = confirm('Are you REALLY sure you want to do this?');
    if (!honestly) return;
    
    $http.delete('/v0/items')
    .then(() => {
      this.getItems(0);
    }, () => {});
  };
});